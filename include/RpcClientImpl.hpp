...
public:
	...
	{
		//srv_->service().post(boost::bind(&Server::on_exchange_update, srv_, params));
		srv_->on_exchange_update(params);
	}

	void on_product_update(const std::shared_ptr<boost::property_tree::ptree>& params)
	{
		//srv_->service().post(boost::bind(&Server::on_product_update, srv_, params));
		srv_->on_product_update(params);
	}
	...
	{
		if(method == "on_exchange_update") {
			this->on_exchange_update(params);
		}else if(method == "on_product_update") {
			this->on_product_update(params);
